...
TSharedPtr<FJsonObject> FUnrealMCPProjectCommands::HandleGetProjectInfo(const TSharedPtr<FJsonObject>& Params)
{
    TSharedPtr<FJsonObject> ResultObj = MakeShared<FJsonObject>();

    ResultObj->SetStringField(TEXT("project_name"), FApp::GetProjectName());
    ResultObj->SetStringField(TEXT("project_directory"), FPaths::ProjectDir());
    ResultObj->SetStringField(TEXT("project_file"), FPaths::GetProjectFilePath());
    ResultObj->SetStringField(TEXT("engine_version"), FEngineVersion::Current().ToString());

    FString CurrentLevel = GWorld ? GWorld->GetMapName() : TEXT("None");
    ResultObj->SetStringField(TEXT("current_level"), CurrentLevel);
